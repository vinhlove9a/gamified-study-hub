...
.notice-panel { border-radius: 1rem; border: 1px solid #fde68a; background: linear-gradient(155deg, rgba(255, 255, 255, 0.88), rgba(255, 247, 237, 0.9)); padding: 0.95rem; }
.status-visual { position: relative; width: 3.6rem; height: 3.2rem; margin-bottom: 0.8rem; }
.status-ring { position: absolute; inset: -0.4rem; border-radius: 1rem; border: 2px solid rgba(251, 146, 60, 0.42); animation: ring 2s ease-out infinite; }
.status-core { position: absolute; inset: 0.4rem 0.4rem 0.2rem; border-radius: 0.7rem; background: linear-gradient(135deg, #fdba74, #fb923c); box-shadow: 0 8px 20px rgba(251, 146, 60, 0.3); }
.status-core--success { background: linear-gradient(135deg, #34d399, #10b981); box-shadow: 0 8px 20px rgba(16, 185, 129, 0.25); }
.status-icon { position: absolute; right: -0.35rem; top: -0.25rem; width: 1.3rem; height: 1.3rem; border-radius: 9999px; background: #fff; color: #b45309; font-size: 0.8rem; font-weight: 700; display: inline-flex; align-items: center; justify-content: center; border: 2px solid #fdba74; }
...
.secondary-btn { width: 100%; border: 1px solid #fdba74; border-radius: 0.9rem; background: rgba(255, 255, 255, 0.8); color: #c2410c; font-size: 0.9rem; font-weight: 600; padding: 0.7rem 0.9rem; cursor: pointer; }
...
@keyframes ring { from{transform:scale(0.85);opacity:1} to{transform:scale(1.5);opacity:0} }
