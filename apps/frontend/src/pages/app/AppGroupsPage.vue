  ...
  <div class="space-y-6">
    ...
    <div v-if="filteredGroups.length > 0" class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
      <article
        ...
        :key="g.id"
        class="group flex cursor-pointer flex-col overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.02] transition-all hover:-translate-y-1 hover:border-white/[0.14]"
        @click="openDetail(g.id)"
      >
        <!-- Gradient cover -->
        <div class="relative h-24 bg-gradient-to-br" :class="cover[g.accent]">
          <div
            class="absolute inset-0 opacity-50"
            style="background: radial-gradient(circle at 25% 20%, rgba(255,255,255,0.28), transparent 55%)"
          />
          <span class="absolute left-4 top-4 flex h-11 w-11 items-center justify-center rounded-xl backdrop-blur" :class="iconChip[g.accent]">
            <AppIcon name="users" class="h-6 w-6" />
          </span>
          <span
            class="absolute right-4 top-4 inline-flex items-center gap-1 rounded-full bg-black/30 px-2 py-1 text-[0.68rem] font-medium text-white/90 backdrop-blur"
          >
        ...
        <div class="flex flex-1 flex-col p-4">
          <div class="flex items-start justify-between gap-2">
            ...
            <span class="shrink-0 rounded-md px-2 py-0.5 text-[0.65rem] font-semibold capitalize" :class="activityTone[g.activity]">
              {{ g.activity }}
            </span>
          </div>
    ...
    <Transition name="drawer">
      <div v-if="detailGroup" class="fixed inset-0 z-40 flex justify-end">
        ...
        <aside class="drawer-panel cosmic-scroll relative z-10 flex h-full w-full max-w-md flex-col overflow-y-auto border-l border-white/[0.08] bg-[#0c0c16]">
          <!-- Cover header -->
          <div class="relative h-28 shrink-0 bg-gradient-to-br" :class="cover[detailGroup.accent]">
            ...
            <button
              type="button"
              class="absolute right-3 top-3 flex h-8 w-8 items-center justify-center rounded-lg bg-black/30 text-white/80 backdrop-blur hover:text-white"
              aria-label="Đóng"
              @click="closeDetail"
            >
              <AppIcon name="x" class="h-5 w-5" />
            </button>
            <div class="absolute -bottom-6 left-5 flex h-14 w-14 items-center justify-center rounded-2xl ring-4 ring-[#0c0c16]" :class="iconChip[detailGroup.accent]">
          ...
          <div class="space-y-5 px-5 pb-6 pt-9">
            ...
            <div>
              <div class="flex items-center gap-2">
                ...
                <span class="inline-flex items-center gap-1 rounded-full bg-white/[0.06] px-2 py-0.5 text-[0.65rem] font-medium text-muted-foreground">
              ...
              <button
                type="button"
                class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold transition-colors"
                :class="
                  detailGroup.joined
            ...
            <div>
              ...
              <ul class="space-y-1.5">
                <li
                  ...
                  :key="m.name"
                  class="flex items-center gap-3 rounded-lg px-2 py-1.5 transition-colors hover:bg-white/[0.03]"
                >
                  <span class="relative">
                    <Avatar :name="m.name" :accent="m.accent" :size="32" />
                    <span
                      class="absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full ring-2 ring-[#0c0c16]"
            ...
            <div>
              ...
              <div v-if="detailGroup.chat.length > 0" class="space-y-2.5 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3">
              ...
              <p v-else class="rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-4 text-center text-xs text-muted-foreground">
            ...
            <div>
              ...
              <ul v-if="detailGroup.docs.length > 0" class="space-y-1.5">
                <li
                  ...
                  :key="doc.title"
                  class="flex items-center gap-3 rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2 transition-colors hover:border-white/[0.12]"
                >
            ...
            <div>
              ...
              <ul class="space-y-1.5">
                <li
                  ...
                  :key="m.name"
                  class="flex items-center gap-3 rounded-lg border border-white/[0.06] bg-white/[0.02] px-3 py-2"
                >
                  <span class="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[0.7rem] font-bold" :class="rankColor[idx] ?? 'bg-white/[0.06] text-muted-foreground'">
                    {{ idx + 1 }}
                  </span>
                  <Avatar :name="m.name" :accent="m.accent" :size="28" />
                  <span class="flex-1 truncate text-sm text-foreground">{{ m.name }}</span>
        ...
        <div class="modal-panel glass-panel relative z-10 w-full max-w-md overflow-hidden rounded-2xl">
          <header class="flex items-center justify-between gap-3 border-b border-white/[0.06] px-5 py-4">
            <div class="flex items-center gap-2.5">
              <span class="flex h-9 w-9 items-center justify-center rounded-xl bg-fuchsia-500/10 text-fuchsia-300">
